...
const DataTable = ({ title, columns, data, onSearch }) => {
  return (
    <div className="data-table-container">
      <div className="table-header" style={{ padding: '1.5rem 1.75rem' }}>
        <h3 style={{ fontSize: '1.125rem', fontWeight: 800, color: 'var(--text-main)', letterSpacing: '-0.02em' }}>{title}</h3>
        <div style={{ display: 'flex', gap: '1rem' }}>
          <div className="search-box">
            <Search className="search-icon" size={16} />
            <input type="text" placeholder="Filter records..." onChange={(e) => onSearch(e.target.value)} />
          </div>
          <button className="btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 16px', borderRadius: '10px' }}>
