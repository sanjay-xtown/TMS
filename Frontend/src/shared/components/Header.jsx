...
const Header = ({ title, subtitle }) => {
  return (
    <div className="px-6 pt-8 pb-4 sticky top-0 z-40 bg-background/90 backdrop-blur-xl border-b border-black/[0.03]">
      <div className="flex items-center justify-between">
        ...
        >
          <div className="w-10 h-10 bg-white/60 rounded-[18px] border border-white/40 flex items-center justify-center shadow-sm relative group active:scale-95 transition-all">
            <Map size={18} className="text-primary group-hover:scale-110 transition-transform" />
            <div className="absolute top-2.5 right-2.5 w-2 h-2 bg-primary rounded-full border-2 border-white animate-pulse" />
          </div>
        </motion.div>
      </div>
    </div>
